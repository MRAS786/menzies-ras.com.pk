...
export class ApiService {
  ...
  getdata(url: string): Observable<any> {
    ...
    return this.httpClient.get(FullUrl).pipe(map(response => {
  ...
  public PostDataWithoutHeader(urls:any, data:any): Observable<any> {
    this.Gv.G_IsRunning = true;
    this.Gv.G_IsRunning = true;
    let FullUrl = this.Gv.serverURL + urls;
    return this.httpClient.post(FullUrl, data).pipe(map(response => {
      this.Gv.G_IsRunning = false;
      return response;
    }),
      catchError(x => this.handleAuthError(x)));
